...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    */
    ...
    NSInteger middleText = [text length] / 2;
    
    NSRange rangeFirstToMiddle = NSMakeRange(0, middleText);
    NSRange rangeMiddleToEnd = NSMakeRange(middleText, [text length] - middleText);
    
    NSLog(@"%@", [text substringWithRange:rangeFirstToMiddle]);
    NSLog(@"%@", [text substringWithRange:rangeMiddleToEnd]);
    
    NSArray *arrayLinesFromText = [text componentsSeparatedByString:@"."];
    NSLog(@"%@", arrayLinesFromText);
    ...
    */
    ...
    NSMutableArray *arrayLinesWithNSString = [[NSMutableArray alloc] init];
    
    for (NSString *string in arrayLinesFromText) {
        NSRange range = [string rangeOfString:@"NSString"];
        if (range.location != NSNotFound) {
            [arrayLinesWithNSString addObject:string];
        }
    }
    
    NSLog(@"%@", arrayLinesWithNSString);
